...
from .position_diff import nor_sep
...
# ---------------------------------------------------
def VSH_deg02(dRA, dDE, e_dRA, e_dDE, RA, DE, cov=None):
...
def VSHdeg02_fitting(dRA, dDE, e_dRA, e_dDE, RA, DE, flog=sys.stdout,
                     cov=None, elim_flag="sigma", N=3.0):
    ...
    if elim_flag is "None" or elim_flag is "none":
        x, sig, cofmat = VSH_deg02(dRA, dDE, e_dRA, e_dDE, RA, DE, cov)
        ind_go = np.arange(dRA.size)

    elif elim_flag is "sigma":
        x, sig, cofmat = VSH_deg02(dRA, dDE, e_dRA, e_dDE, cov, RA, DE)
        # Iteration.
        num1 = 1
        num2 = 0
        while(num1 != num2):
        ...
        dRAn, dDEn, e_dRAn, e_dDEn, covn, RAn, DEn = find_good_obs(
            dRA, dDE, e_dRA, e_dDE, cov, RA, DE, ind_go)
        x, sig, cofmat = VSH_deg02(dRAn, dDEn, e_dRAn, e_dDEn, covn,
                                   RAn, DEn)

        print('# Number of sample: %d' % dRA.size, file=flog)
